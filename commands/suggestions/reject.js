...
module.exports = {
    ...
    aliases: [],
    permissions: [],
    description: "suggestion reject command",
    ...
    async execute(client, message, args, cmd, Discord, text, profileData, serverData){
        ...
        const channel = message.guild.channels.cache.get(serverData.deniedChannel);
        ...
        const argsEmbed = new Discord.MessageEmbed() .setDescription(text.translate["reject"]["NO_ID"][serverData.language]) .setColor('DARK_RED') .setTimestamp();
        ...
        if(suggestionData.comment !== "") embed.addField(`${text.translate["reject"]["COMMENT"][serverData.language]}`, suggestionData.comment)
